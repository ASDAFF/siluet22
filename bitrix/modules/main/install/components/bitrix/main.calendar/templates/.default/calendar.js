...
function JCCalendar()
{
	...
	this.FormatDate = function(date, format)
	{
		...
		str = str.replace(/H/ig, this.Number(date.getHours()));
	...
	this.ParseDate = function(str)
	{
		if (phpVars.cutMonth)
		{
			var wordMonthCut = top.phpVars.cutMonth;
			var wordMonth = top.phpVars.month;
		}
		else
		{
			var wordMonthCut = {jan: 1, feb: 2, mar: 3, apr: 4, may: 5, jun: 6, jul: 7, aug: 8, sep: 9, oct: 10, nov: 11, dec: 12};
			var wordMonth = {january: 1, february: 2, march: 3, april: 4, may: 5, june: 6, july: 7, august: 8, september: 9, october: 10, november: 11, december: 12};
		}

		var aDate = str.split(/[^a-z0-9]/ig);
		var aFormat = phpVars.FORMAT_DATE.split(/[^a-z0-9]/ig);
		if(aDate.length > aFormat.length)
			aFormat = phpVars.FORMAT_DATETIME.split(/[^a-z0-9]/ig);
		...
		if (aResult['MM'] == undefined && (aResult['M'] != undefined || aResult['MMMM'] != undefined))
		{
			if (aResult['M'] != undefined)
			{
				...
				{
					aResult['MM'] = parseInt(wordMonthCut[aResult['M'].toLowerCase()], 10);
				}
				else
				{
			...
			else
			{
				...
				{
					aResult['MM'] = parseInt(wordMonth[aResult['MMMM'].toLowerCase()], 10);
				}
				else
				{
