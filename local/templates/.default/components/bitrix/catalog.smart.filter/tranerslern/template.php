...
?>
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter" id="formtran">
            <ul class="b-checkbox">
			...
			<?endforeach;?>
                ...
                <?foreach($arItem["VALUES"] as $val => $ar):?><?if($ar["CHECKED"]){$countcheck++;}?><?endforeach;?>
                            <li class="b-checkbox__item checkbox">
                                <input
                                        ...
                                        onclick="smartFilter.click(this)"
                                   <?if($countcheck == 0):?>checked<?endif;?>
                                />
                                <label class="checkbox__label" for="all-brands">
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#all-brands').on('change', function(e) {
            ...
            $(this).closest('form').find('.checkbox__input:not(#all-brands)').prop('checked', false);

            $(this).closest('form').find('#set_filter').click();
        });
